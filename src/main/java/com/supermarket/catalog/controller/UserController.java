...
public class UserController {
    ...
    public UserResponse get(@PathVariable UUID id) {
        ...
        return new UserResponse(user.getId(), user.getUsername(), user.getEmail(), user.getJoinedAt());
    }

    @PutMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void update(@PathVariable UUID id, @RequestBody @Valid UpdateUserRequest request) {
        userService.updateUser(id, request.username(), request.email());
    }
